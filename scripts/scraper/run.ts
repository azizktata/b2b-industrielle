...
// Future scrapers — uncomment as you build them:
...
const SCRAPERS: Record<string, () => Promise<BrandResult>> = {
  ...
  // sectoriel: async () => {
  //   const { products, guides, catalogues } = await scrapeSectoriel()
  //   return { products, guides, catalogues }
  // },
  // mival: async () => {
  ...
  // },
  // sferaco: async () => {
  //   const { products, guides, catalogues } = await scrapeSferaco()
  //   return { products, guides, catalogues }
  // },
  // adca: async () => {
...
async function main() {
  ...
  const mergedProducts = mergeById(existing.products, freshProducts, brandsToRun)
  const mergedGuides = mergeById(existing.guides, freshGuides, brandsToRun)
  const mergedCatalogues = mergeById(existing.catalogues, freshCatalogues, brandsToRun)
...
function mergeById<T extends { id: string; marque: string }>(
  ...
  return [...kept, ...fresh].sort((a, b) =>
    a.marque.localeCompare(b.marque) || a.id.localeCompare(b.id)
  )
}
